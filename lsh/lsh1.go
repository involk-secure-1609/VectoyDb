...
type cosineLshParam struct {
	dim         int          // Dimensionality of the input data points.
	l           int          // Number of hash tables to use.
	m           int          // Number of hash functions (hyperplanes) used in each hash table to create a hash key.
	hyperplanes [][]float64  // The set of randomly generated hyperplanes used for hashing.
	h           int          // Total number of hyperplanes (l * m).
	dFunc       DistanceFunc // Function to calculate the distance between vectors.
}
...
func newCosineLshParam(dim, l, m, h int, hyperplanes [][]float64) *cosineLshParam {
	return &cosineLshParam{
		...
		dFunc:       euclideanDistSquare, // Use squared Euclidean distance as the default distance function.
	}
}
...
func (clsh *cosineLshParam) hash(point []float64) []hashTableKey {
	...
	for i := range hvs {                           // For each hash table...
		...
		for j := range clsh.m {   // For each hash function in this hash table...
			s[j] = uint8(simhash.sig[i*clsh.m+j]) // Take 'm' bits from the simhash signature to form the hash key.
...
func NewCosineLsh(dim, l, m int) *CosineLsh {
	...
	return &CosineLsh{
		cosineLshParam: newCosineLshParam(dim, l, m, h, hyperplanes), // Initialize LSH parameters.
...
func (index *CosineLsh) Insert(point []float64, extraData string) {
	...
	wg.Add(len(index.tables))     // Increment WaitGroup counter by the number of hash tables.
	for i := range index.tables { // Iterate through each hash table.
		hv := hvs[i]                          // Get the hash key for the current hash table.
		table := index.tables[i]              // Get the current hash table.
		go func(table hashTable, hv uint64) { // Launch a goroutine for concurrent insertion.
			...
			vectorID := atomic.AddUint64(&index.nextID, 1)                                          // Atomically increment the point ID counter and get the new ID.
...
func (index *CosineLsh) Query(q []float64, maxResult int) []QueryResult {
	...
	for i, table := range index.tables { // Iterate through each hash table and corresponding hash key.
	...
	for _, value := range seen {                   // Iterate through the unique points found in the hash tables.
		dist := index.dFunc(q, value.Vector)       // Calculate the distance between the query point and the candidate point.
...
func (index *CosineLsh) toBasicHashTableKeys(keys []hashTableKey) []uint64 {
	basicKeys := make([]uint64, index.cosineLshParam.l) // Create a slice to store the converted uint64 keys.
